...
"""
    ...
    etc if there is an error) before data goes to serializer --> in serializer we validations
    everal customized restrictions (error raises if there is any) -> check integrity in data
...
class TaskSerializer(serializers.ModelSerializer):
    ...
    def get_assignee_name(self, obj):
        return (
            obj.assignee.username if obj.assignee else None
        )  # Handle potential null assignee
    ...
    def validate(self, data):
        ...
        Only task owner can update a task detail and approve a task (after it has
        been put to review)
        """
            ...
            for field in ["name", "description", "output"]:
                if field in data:
                    if not user_profile.is_employer:
                        errors[field] = (
                            "Your do not have the permission to change this field."
                        )
            ...
            if "assignee" in data:
                ...
                if task and task.assignee:
                    if task.assignee != user:
                        errors["assignee"] = (
                            "Only the current task assignee can change this field."
                        )
            ...
            if "is_submitted" in data:
                if user_profile.is_employer:
                    ...
                    if task.status == Task.Status.NOT_STARTED:
                        errors["is_submitted"] = (
                            "Cannot submit before the task is started."
                        )
            ...
            if "is_approved" in data:
                if not user_profile.is_employer:
                    ...
                    if (
                        ...
                        errors["is_approved"] = (
                            "Cannot approve before the task is submitted."
                        )
                    elif task.owner != user:
                        errors["is_approved"] = (
                            "Only the task owner can change this field."
                        )
    ...
    def validate(self, data):
        ...
        if request and request.method in ["POST", "PUT", "PATCH"]:
            ...
            if user_password:
                ...
                if not any(
                    char in "!@#$%^&*()_+-=[]{}|;:,.<>?/`~" for char in user_password
                ):
                    errors["password"].append(
            ...
            for field in ["is_staff", "is_superuser"]:
                if field in data:
                    if request.method == "POST":
                        ...
                        errors[field].append(
                            "Modification of the state of staff and superuser is not allowed"
                        )
            ...
            if task_instance and new_task != task_instance:
                if not new_task.owner == request_user:
                    raise serializers.ValidationError(
                        {"task": "You can only relink properties to tasks you own."}
                    )

            if new_task and not task_instance:
                if data["task"].owner != request_user:
                    raise serializers.ValidationError(
                        {"task": "You can only link properties to your own tasks."}
                    )
...
class UserPropertySerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        As an authorized user, you can only link a property to yourself, you cannot created linkage
        for others
        ...
        if request:
            if request.method == "POST":
                ...
                if data["user"] and data["user"] != request_user:
                    raise serializers.ValidationError(
                        {
                            "user": "You can only create linkage to a property for yourself."
                        }
                    )
                ...
                if new_user and new_user != self.instance.user:
                    raise serializers.ValidationError(
                        {"user": "You cannot link the property to someone else."}
                    )
...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        As an authorized user, you can only have one profile that is prohibited from modifying
        once created.
        """
        ...
        if request and request.method == "POST":
            ...
            if data["user"] and data["user"] != request_user:
                raise serializers.ValidationError(
                    {"user": "You can only set profile for yourself."}
                )
