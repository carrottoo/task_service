...
class UserPropertyTests(APITestCase):
    ...
    def test_list_retrieval(self):
        ...
        )  # for non superusers, they cannot see the properties linked to themselves
    ...
    def test_detail_retrieval_by_id(self):
        ...
        )  # A normal user cannot access details of properties likned to other users
        self.assertEqual(
            response.data["detail"]["message"],
            "You do not have access to the requested information",
        )
    ...
    def test_user_property_creation(self):
        """
        Test POST request to create a new user property object (linking one property tag to a user),
        only authenticated users with employee profile are allowed for such action. Moreover, you
        can only link a property to yourself
        """

        create_url = reverse("user_property-list")

        data = {"user": self.user_employee_1.id, "property": self.property_2.id}
        ...
        # Authenticated user and with profile of employee, however the user is trying to set property
        # for others -> should fail
        self.client.login(
            username=self.user_employee_1.username, password="myuniquepassword"
        )
        ...
        self.assertEqual(
            response.data["errors"]["user"]["message"],
    ...
    def test_user_property_update(self):
        """
        Test PUT request to update a user property object, you are free to modify the linked
        property and your interest once you are authenticated but you are not allowed to relink
        your property tag to someone else
        """

        update_url = reverse(
            "user_property-detail", kwargs={"pk": self.user_property_1.id}
        )
        ...
        # Authenticated user and also the user him/herself, however the user wants to link
        # the property to another user -> should fail
        data["user"] = self.user_employee_2.id
        response = self.client.put(update_url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data["error_count"], 1)
        ...
        # Authenticated user and also the user him/herself, however the user wants to
        # null the user/property/is_interested or modified to wrong types -> should fail
    ...
    def test_user_property_partial_update(self):
        """
        Test PUT request to update a user property object, you are free to modify the
        linked property and your interest once you are authenticated but you are not
        allowed to relink your property tag to someone else
        """
        ...
        # Authenticated user and also the user him/herself, trying to partially null
        # is_interested  -> should fail
        ...
        # Authenticated user and also the user him/herself, however the user wants to
        # link the property to another user -> should fail
        data = {"user": self.user_employee_2.id}
        response = self.client.patch(update_url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data["error_count"], 1)
    ...
    def test_task_property_deletion(self):
        """
        Test DELETE request to delete a user property by its id, only the user itself
        are allowed for such an action
        """
