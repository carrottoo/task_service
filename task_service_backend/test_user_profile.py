...
class UserProfileTests(APITestCase):
    ...
    def test_list_retrieval(self):
        ...
        )  # for non superusers, they cannot see other users' profiles
    ...
    def test_detail_retrieval_by_id(self):
        ...
        self.assertEqual(
            ...
            "You do not have access to other user's profile",
        )
        ...
        )  # A normal user cannot access profiles of other users
        self.assertEqual(
            response.data["detail"]["message"],
            "You do not have access to other user's profile",
        )
    ...
    def test_user_profile_creation(self):
        """
        Test POST request to create a new user profile, you can only create a
        profile for yourself and you can only have exactly one profile
        """
        ...
        # One to one relationship between user and user profile, uniqueness is
        # ensured on the database level
        response = self.client.post(create_url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data["error_count"], 1)
        self.assertTrue("user" in response.data["errors"].keys())
    ...
    def test_user_profile_update(self):
        """
        Test PUT request to partically update a user profile, this endpoint has been
        ...
        data = {"user": self.user_1.id, "profile": False}

        self.client.login(username=self.user_1.username, password="mysecretpassword")
        response = self.client.put(update_url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertEqual(
            response.data["error"], "UserProfile cannot be modified once set."
        )

    def test_user_profile_partial_update(self):
        """
        Test PATCH request to partically update a user profile, this endpoint has been
        disabled to maintain the data consistency in the database
        """
    ...
    def test_user_profile_deletion(self):
        """
        Test DELETE request to delete a user profile by its id, only the user itself
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.data["error"], "Cannot delete profile while the user exists."
        )
