...
class UserService:
    ...
    def authenticate_by_name(username: str, password: str) -> tuple:
        """
        Authenticate the user by username and password. It first checks if the username exists
        and gives error message when the username is not found in the database. Then it
    ...
    def authenticate_by_email(email: str, password: str) -> tuple:
        """
        Authenticate the user by email address and password. It first checks if the email address
        exists and gives error message when the email address is not found in the database. Then
        it authenticates the user with the given password.
        """
...
class TaskService:
    ...
    We do not handle the existence check and authentication of a request/id
    related to a task or user model in the servce, we take valid task and user
    ...
    def get_user_interested_properties(user: User) -> set:
        ...
        interested_properties = set(
            user.user_properties.filter(is_interested=True).values_list(
    ...
    def get_user_liked_tasks(user: User) -> set:
        ...
        liked_tasks = set(
            user.user_behaviors.filter(is_like=True).values_list("task_id", flat=True)
        )
        return liked_tasks
    ...
    def get_user_completed_tasks(user: User) -> set:
        ...
        completed_tasks = set(
            user.assigned_tasks.filter(status=Task.Status.DONE).values_list(
                "id", flat=True
            )
        )
    ...
    def get_task_properties(task: Task) -> set:
        ...
        linked_properties = set(
            task.task_properties.all().values_list("property_id", flat=True)
        )
    ...
    def calc_task_similarity(task_1: Task, task_2: Task) -> float:
        """
        Calculates task similarity based on the task description. We are interested in
        the topical similarity and use the TF-IDF vectors and cosine similarity
    ...
    def get_history_based_score(self, user: User, task: Task) -> float:
        """
        Calculates the score of a task based on the user's task history (completed tasks).
        It calculates the average similarity of the tasks to all completed tasks of the user.
        If the task is very similar to the completed tasks, its average similarity should be
        high.

        :param user: an instance of the User model
        :param task_id: the id of an instance of the Task model

        return: the value of the score of a task for the given user based on user's history of
        tasks he/she has completed
        """
    ...
    def get_behavior_based_score(self, user: User, task: Task) -> float:
        """
        Calculates the score of a task based on the user's behavior (what task he/she has liked).
        We use similar approach as the calculation of histroy based task score by calculating
        the average similarity of tasks to all liked tasks of the user. If the task is very
        similar to the liked tasks of the user, its average similarity should be high.

        :param user: an instance of the User model
        :param task: an instance of the Task model

        return: the value of the score of a task for the given user based on user's behavior about
        what tasks he/she has liked
        """
    ...
    def get_user_task_rank(self, user: User) -> list:
        """
        Performs ranking to all the tasks for a user based on the user's interests, task completion
        ...
        normalized_interest_scores = {
            task_id: score / interest_score_max if interest_score_max else 0
            for task_id, score in interest_scores.items()
        }
        ...
        normalized_history_score = {
            task_id: (
                [(score - history_score_mean)] / history_score_std
                if history_score_std
                else 0
            )
            for task_id, score in history_scores.items()
        }
        ...
        normalized_behavior_score = {
            task_id: (
                [(score - behavior_score_mean)] / behavior_score_std
                if behavior_score_std
                else 0
            )
            for task_id, score in history_scores.items()
        }
        ...
        for task in all_active_task:
            final_scores[task] = (
                normalized_interest_scores[task]
                + normalized_history_score[task]
                + normalized_behavior_score[task]
            )
