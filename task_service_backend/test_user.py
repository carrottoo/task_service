...
class UserTests(APITestCase):
    ...
    def test_list_retrieval(self):
        """
        Test GET request for retrieving list of users. For superusers, they can see all
        the users in the db. However, for normal users they can only see themselves
        """
    ...
    def test_detail_retrieval_by_id(self):
        """
        Test GET request to retrieve a user's detail by its id. Except for superusers,
        users can only retrieve their own profile by id
        """
    ...
    def test_normal_user_creation(self):
        ...
        for field in ["username", "email", "password"]:
            self.assertEqual(
                response.data["errors"][field]["message"], "This field is required."
            )
        ...
        data = {"username": "test name", "email": "email", "password": "12345"}

        response = self.client.post(create_url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data["error_count"], 3)
        self.assertEqual(
            response.data["errors"]["username"]["code"], "invalid"
        )  # may contain only letters, numbers, and @/./+/-/_ characters
        self.assertEqual(response.data["errors"]["email"]["code"], "invalid")
        self.assertEqual(
            response.data["errors"]["password"]["code"], "min_length"
        ...
        response = self.client.post(create_url, data, format="json")
        # this updated data will trigger model level validators and error of violating default max length
        # will be raised before it goes to the customized logics in validate() in serializer  -> total errors
        # raised should be 1
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data["error_count"], 1)
        self.assertEqual(
            response.data["errors"]["username"]["code"], "max_length"
        )  # default max length of username in Django User model is 150

        data["username"] = "valid_name"
        response = self.client.post(create_url, data, format="json")
        # updated data will pass the model level validators and in customized validate() password
        # doesn't follow the rules of containing at least one letter/  specail symbols, 2 errors
        # will be raised
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        ...
        self.assertEqual(
            newly_created_user.is_superuser, False
        )  # by default is_staff and is_superuser are set to false
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.data["errors"]["is_staff"]["message"],
            "Creation of staff and superuser is not allowed.",
        )
    ...
    def test_user_update(self):
        ...
        # You can have password in this data json but the password wouldn't be updated.
        # To update password, use the update password endpoint
        data = {
            "username": "test name",
            "email": "email",
            "password": "abc",
    ...
    def test_user_authentication(self):
        ...
        data = {"username": "some one", "password": "abcedf"}

        response = self.client.post(url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data["error"], "Username does not exist.")

        data = {"email": "user@gmail.com", "password": "mysecretpassword"}
