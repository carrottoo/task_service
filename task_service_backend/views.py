...
"""
    Data fields' non-nulless, data type are checked (error raises and return 'bad request' 
    etc if there is an error) before data  goes to serializer --> in serializer we validation 
    several customized restrictions (error raises if there is any) -> check integrity in data 
    (e.g. duplicated records if uniqueness is required) when saving the instance after performing 
    the action in view (raise error if there is any)
"""
...
class CustomPageNumberPagination(PageNumberPagination):
    ...
    By default, display 15 items per page. However user can override the page size via a query
    paramter. When user wants to override the default page size, a maximum size limit of 100 will
    apply
    """
...
class TaskViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        """
        A superuser can access all tasks, a employee can access all active non-assigned tasks and
        tasks assigned to themselves, a employer can access tasks he has created
        """
        ...
        if request.user.is_superuser:
            ...
            queryset = (
                []
            )  # display empty set if the user doesn't have set a profile or not logged in
    ...
    def retrieve(self, request, *args, **kwargs):
        """
        A superuser can retrieve any task by id, a employee can retrieve any task that is assigned
        to him/her or an unasssigned active task, a employer can retrieve any task that is owned by
        him/her
        """
    ...
    def recommend(self, request):
        ...
        serializer = TaskSimpleSerializer(
            ranked_tasks, many=True, context={"request": request}
        )
        return Response(serializer.data)
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        ...
        if self.action in ["update", "partial_update"]:
            # PUT and PATCH request to update a user's details: only the user himself/herself
            # is allowed
    ...
    def list(self, request, *args, **kwargs):
        """
        Superusers can see all the users in the system but for others they should get a filtered
        result (they can only see themselves if they are a valid user in the system)
        """
...
class PropertyViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        """
        A superuser and a user with employee profie can see all the properties in DB, however a
        normal user with employer profile or no profile set can only see the propeties he/she has
        created
        """
        if request.user.is_superuser:
            queryset = Property.objects.all()
    ...
    def update(self, request, *args, **kwargs):
        ...
        return Response(
            {"error": "Property cannot be modified once created"},
...
class TaskPropertyViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        """
        A superuser can see all the properties in DB, however a normal user can only see the
        propeties of the task he/she has owned
        """
        if request.user.is_superuser:
            queryset = TaskProperty.objects.all()
        else:
...
class UserPropertyViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        """
        A superuser can see all the user properties in DB, however a normal user can only
        see the propeties he/she have linked to themselves
        """
        if request.user.is_superuser:
            queryset = UserProperty.objects.all()
        else:
    ...
    def retrieve(self, request, *args, **kwargs):
        """
        For non superuser, he/she can only retrieve a property tag that has been linked to
        him/herself
        """
...
class UserProfileViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        """
        A superuser can see all the user profiles in DB, however a normal user can only see
        his/her own profile
        """
        if request.user.is_superuser:
            queryset = UserProfile.objects.all()
    ...
    def update(self, request, *args, **kwargs):
        ...
        return Response(
            {"error": "UserProfile cannot be modified once set."},
            status=status.HTTP_403_FORBIDDEN,
        )
    ...
    def destroy(self, request, *args, **kwargs):
        """
        Override the destroy method to prevent deletion of the UserProfile if the associated User
        exists.
        """
        ...
        if User.objects.filter(id=user.id).exists():
            ...
            return Response(
                {"error": "Cannot delete profile while the user exists."},
...
class UserBehaviorViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        """
        A superuser can see all the user behaviors in DB, however normal users can only
        see their own behaviors
        """
        if request.user.is_superuser:
            queryset = UserBehavior.objects.all()
        else:
