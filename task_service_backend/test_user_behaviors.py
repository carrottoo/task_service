...
class UserBehaviorsTests(APITestCase):
    ...
    def setUp(self) -> None:
        ...
        self.behavior_1 = UserBehavior.objects.create(
            user=self.user_employee_1, task=self.test_task_1, is_like=True
        )
    ...
    def test_user_behavior_creation(self):
        """
        Test POST request to create a new user behavior, you can only create behaviors for
        yourself and you can only have one behavior per task
        """
        ...
        self.assertEqual(response.data["error_count"], 1)
        self.assertEqual(
            response.data["errors"]["non_field_errors"]["message"],
        ...
        self.assertEqual(response.data["error_count"], 1)
        self.assertEqual(
            response.data["errors"]["task"]["message"], "This field is required."
        )

        data = {"user": self.user_employee_2.id, "task": "wrong data type"}
        response = self.client.post(create_url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data["error_count"], 1)
        self.assertEqual(
    ...
    def test_user_behavior_update(self):
        """
        Test PUT request to update an existing user behavior, you are only allowed to
        modify behaviors for yourself
        """
        ...
        # Remapped your behavior to another task, as long as the new targetted
        # task hasn't been mapped before
        ...
        self.assertEqual(response.data["error_count"], 1)
        self.assertEqual(
            response.data["errors"]["task"]["message"], "This field may not be null."
        )
    ...
    def test_user_behavior_partial_update(self):
        ...
        # Authenticated user and also the user him/herself, however the user wants to link the
        # property to another user -> should fail
    ...
    def test_user_behavior_deletion(self):
        """
        Test DELETE request to delete an existing user behavior, you are only allowed to
        delete your own behaviors
        """
